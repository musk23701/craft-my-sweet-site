...
const SkoolPopupWhite = () => {
  ...
  useEffect(() => {
    ...
    localStorage.removeItem("skool-popup-white-seen");
    
    const hasSeenPopup = localStorage.getItem("skool-popup-white-seen");
    
    if (!hasSeenPopup) {
      const timer = setTimeout(() => {
        setIsOpen(true);
      }, 6000); // 6 seconds for white version to compare
  ...
  const handleClose = () => {
    setIsOpen(false);
    localStorage.setItem("skool-popup-white-seen", "true");
  };
